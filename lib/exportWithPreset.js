...
async function exportWithPreset(sequence, outputPath, presetPath, exportArea) {
    ...
    if (!presetPath) {
        console.error('exportWithPreset: no presetPath provided — the .epr/.sqpreset could not be resolved. Check payloads/v{version}/ contains the expected preset file.');
        return false;
    }
